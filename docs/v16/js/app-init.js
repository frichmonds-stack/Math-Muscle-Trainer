...
function getHomeWorkoutSettings(operation, questionTarget, correctTarget = null) {
  return {
...
function startHomeWorkout(operation, questionTarget, correctTarget = null) {
  applySettingsSnapshot(getHomeWorkoutSettings(operation, questionTarget, correctTarget));
  toggleSetupFields();
  const settings = {
    ...readSettings(),
    ...(correctTarget ? { correctTarget } : {}),
...
function handleHomeDashboardClick(event) {
  ...
  if (routineButton) {
    event.preventDefault();
    startHomeWorkout(routineButton.dataset.homeRoutineOperation, 20, 5);
    return;
  }
  ...
  if (operationButton) {
    event.preventDefault();
    startHomeWorkout(operationButton.dataset.homeOperation, 20);
    return;
  }
