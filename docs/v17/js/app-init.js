...
function getAdaptiveHomeBand(operation) {
  ...
  if (accuracy >= 0.9 && (averageMs === null || averageMs <= 4500)) {
    return "hard";
  }
  if (accuracy >= 0.8) {
    return "medium";
...
function getAdaptiveHomeOperationSettings(operation) {
  ...
  if (operation === "addition") {
    settings.additionDifficulty = difficultyByBand[band][0];
    settings.additionDifficulties = difficultyByBand[band];
  }
  if (operation === "subtraction") {
    settings.subtractionDifficulty = difficultyByBand[band][0];
    settings.subtractionDifficulties = difficultyByBand[band];
  }
  if (operation === "multiplication" || operation === "division") {
    Object.assign(settings, factorRangeByBand[band]);
  }
