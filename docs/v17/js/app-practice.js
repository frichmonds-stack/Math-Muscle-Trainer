...
function renderPracticeProgress() {
  ...
  if (settings.correctTarget) {
    fillRatio = Math.min(state.session.correct / settings.correctTarget, 1);
    progressLabel = `${state.session.correct} / ${settings.correctTarget} correct`;
  } else if (settings.sessionType === "question-goal" || settings.sessionType === "isolation") {
    fillRatio =
...
function isSessionComplete() {
  if (state.settings.correctTarget) {
    return state.session.correct >= state.settings.correctTarget;
  }
